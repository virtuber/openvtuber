...
def detect_iris(frame, marks, side="left"):
    ...
    return:
        ...
        x_amount: how much the iris is towards the left. 0 means totally left (min_x), 1 is totally right (max_x)
        y_amouny: how much the iris is towards the top. 0 means totally top (min_y), 1 is totally right (min_y)
    """
    ...
    try:
        ...
        eye = cv2.cvtColor(eye, cv2.COLORRGB2GRAY)
...
def mouth_distance(mouth):
    return nl.linalg.norm(mouth[0] - mouth[4])
...
def infer_image(image):
    ...
    dlib_model_path = os.path.join("assets/shape_predictor_68_face_landmarks.dat")
